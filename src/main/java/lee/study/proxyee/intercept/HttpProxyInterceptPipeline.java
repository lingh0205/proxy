...
public class HttpProxyInterceptPipeline implements Iterable<HttpProxyIntercept> {
  ...
  public void beforeRequest(Channel clientChannel, HttpRequest httpRequest, RequestProto requestProto) throws Exception {
    ...
    if (this.pos1 < intercepts.size()) {
      ...
      intercept.beforeRequest(clientChannel, this.httpRequest, requestProto, this);
    }
    this.pos1 = 0;
  }

  public void beforeRequest(Channel clientChannel, HttpContent httpContent, RequestProto requestProto) throws Exception {
    if (this.pos2 < intercepts.size()) {
      HttpProxyIntercept intercept = intercepts.get(this.pos2++);
      intercept.beforeRequest(clientChannel, httpContent, requestProto, this);
    }
    this.pos2 = 0;
  }
